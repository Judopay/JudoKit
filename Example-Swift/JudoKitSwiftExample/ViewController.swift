...
class ViewController: UIViewController, PKPaymentAuthorizationViewControllerDelegate, UITableViewDelegate, UITableViewDataSource {
    ...
    func paymentOperation() {
        JudoKit.payment(judoID, amount: Amount(35.0, currentCurrency), reference: Reference(consumerRef: "consumer reference", paymentRef: "payment reference"), completion: { (response, error) -> () in
            if let _ = error {
    ...
    func preAuthOperation() {
        JudoKit.preAuth(judoID, amount: Amount(40, currentCurrency), reference: Reference(consumerRef: "consumer reference", paymentRef: "payment reference"), completion: { (response, error) -> () in
            if let _ = error {
                self.alertController = UIAlertController(title: "Error", message: "there was an error performing the operation", preferredStyle: .Alert)
                self.alertController!.addAction(UIAlertAction(title: "OK", style: .Cancel, handler: nil))
                return // BAIL
    ...
    func createCardTokenOperation() {
        JudoKit.registerCard(judoID, amount: Amount(1), reference: Reference(consumerRef: "consumer reference", paymentRef: "payment reference"), completion: { (response, error) -> () in
            if let _ = error {
                self.alertController = UIAlertController(title: "Error", message: "there was an error performing the operation", preferredStyle: .Alert)
                self.alertController!.addAction(UIAlertAction(title: "OK", style: .Cancel, handler: nil))
                return // BAIL
    ...
    func repeatPaymentOperation() {
        if let cardDetails = self.cardDetails, let payToken = self.paymentToken {
            JudoKit.tokenPayment(judoID, amount: Amount(30), reference: Reference(consumerRef: "consumer reference", paymentRef: "payment reference"), cardDetails: cardDetails, paymentToken: payToken, completion: { (response, error) -> () in
                if let _ = error {
                    self.alertController = UIAlertController(title: "Error", message: "there was an error performing the operation", preferredStyle: .Alert)
                    self.alertController!.addAction(UIAlertAction(title: "OK", style: .Cancel, handler: nil))
                    return // BAIL
    ...
    func repeatPreAuthOperation() {
        if let cardDetails = self.cardDetails, let payToken = self.paymentToken {
            JudoKit.tokenPreAuth(judoID, amount: Amount(30), reference: Reference(consumerRef: "consumer reference", paymentRef: "payment reference"), cardDetails: cardDetails, paymentToken: payToken, completion: { (response, error) -> () in
                if let _ = error {
                    self.alertController = UIAlertController(title: "Error", message: "there was an error performing the operation", preferredStyle: .Alert)
                    self.alertController!.addAction(UIAlertAction(title: "OK", style: .Cancel, handler: nil))
                    return // BAIL
